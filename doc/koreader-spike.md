...
Wayland kiosk on the device. A from-source Guix build is a real
project (~30 vendored third-party deps behind a network-fetching build
system) and stays future work.**
...
- aarch64 cross-build: same recipe, `--target=aarch64-linux-gnu`
  (patchelf edits foreign ELFs fine; the interpreter path comes from
  the cross glibc input).
...
## 3. Device integration plan (next steps, in order)

1. **Compositor**: `cage` (wlroots kiosk; in Guix at 0.2.1) running
   KOReader as its single client, with `WLR_RENDERER=pixman` (software
   rendering on the EBC DRM driver's dumb buffers — the same driver
   class the community runs sway/GNOME on). This follows hrdl's proven
   `pinenote-dist` architecture (sway + koreader-bin) but smaller.
2. **A `reader` flavor** (`pinenote/systems/`): cage+koreader session
   on tty1 in kiosk mode, on top of the usb-console flavor so the
   gadget console remains the escape hatch. Needs glibc UTF-8 locale
   (the launcher exports `LC_ALL=en_US.UTF-8`).
3. **Input**: pen/touch arrive via libinput inside wlroots; KOReader
   sees standard SDL pointer events. The known upstream gap #14694
   (stylus tagged as finger) applies when we get there.
4. **Refresh policy**: initially every SDL present = full-plane damage
...
5. **Hardware session goal**: cage+KOReader on the panel, page turn,
   verify refresh behavior and pen/touch — rides along with the
   already-queued cherry-pick validation (`doc/status.md`).

What can be proven offline before that session: the flavor builds; the
session service wiring (chroot/mock-helper inspection); KOReader runs
offscreen on x86_64; possibly cage+KOReader under qemu-virt once the
udev deadlock is fixed (not before). What cannot: panel optics, e-ink
refresh feel, pen latency — hardware-only, as ever.

## 4. The from-source path (future work, evidence)
