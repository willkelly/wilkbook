...
| ~~`pinenote/tools/ebc-barrier`~~ | — | Retired by the embrace sweep S2 (2026-09-03, `ae1f9bb`) together with the shipping-only driver it exercised: the paint/barrier/restore diagnostic and `make ebc-barrier-check` are gone from the tree; the pure barrier and sleep-frame logic tests that remain are `pinenote/tools/power/test-ebc-barrier.lua` and `test-ebc-sleep-frame.lua` | — |
...
| `pinenote/tools/platform-controls` | 1 | Executes the accepted broker protocol, Wi-Fi ownership, driver/frontlight, and Phase 2 service-wiring gates. It pins that Shepherd respawns the broker, `reader-session` waits for both its readiness marker and immutable-identity uinput node, the production driver no longer consumes the early validation gate, and the reader flavor no longer instantiates the legacy autosuspend writer.  Since 2026-09-02 also **`test-quiesce.lua`** (issue #42): the broker's EBC quiesce is a pure module chosen by driver capability — the shipping driver's `REFRESH_BARRIER` where the driver registers it, otherwise a wait for the EBC's interrupt line (`fdec0000.ebc`, the row the lab's frame-clock counts) to hold still for 250 ms — pinned through a mid-refresh panel that goes idle, one that never does (times out as `EBC busy`), a missing interrupt line (refuses), and an already-idle panel (still proves the silence); plus the degraded-mode start with no physical inputs (rung 4) | `make platform-controls-check` |
...
| EBC ioctl roster preflight | 1 | The two drivers do not share an ioctl table (the shipping driver adds `REFRESH_BARRIER` at DRM command `0x03`; hrdl's direct driver registers `RECT_HINTS` there) and DRM dispatches on the command number, so a tool built for one driver lands in a *different* handler on the other — issue #42: every suspend on the direct image aborted with EFAULT inside `ioctl_rect_hints`. Reconstructs `rockchip_ebc_drm.h` for BOTH drivers from the patches (the direct header by applying the direct-mode hunks with every context line verified), computes every `DRM_IOCTL_ROCKCHIP_EBC_*` number the way `_IOC()` does with a C layout engine for the request structs (fixed-width ints, `bool`, aarch64 pointers, embedded `drm_mode_rect`), **proves the calculator against seven glass-proven constants**, then checks every hardcoded EBC ioctl in the on-device roster against the driver(s) that code is declared for, printing what the same command number means on the other driver; pins that the suspend broker reaches the shipping-only barrier only through its driver probe; positive control: the barrier literal declared for both drivers MUST be rejected | `make ebc-ioctl-roster-check` |
| QEMU update flow | 4u | `doc/update-path.md` end to end with no glass: boot generation A from a rootfs with a synthetic data partition carrying a harness ssh key, a fixed host key (so the guest's identity is pinnable before its first boot and survives the kexec) and this workstation's guix signing key; send generation B (the hostname variant `update-flow-generation-b.scm`, built from the tree — the rig refuses a generation whose shipped helper is not the tree's) into the guest over a forwarded port as guix's signed nar stream of only the paths the guest is missing (`guix archive --missing` → `--export` → `--import` over plain OpenSSH); `add` (menu rendered, `DEFAULT` still A); kexec `trial`; the guest answers as B (`/run/current-system`, hostname); `health`; `promote` (`DEFAULT` = B, Guix's profile link follows); `pin` A, `prune --keep 1` (A survives, its marker and profile link intact), `unpin` A; then `trial` A, `health`, `promote` A — rollback is the same move — and `prune --keep 0`, nothing pinned, takes B (the positive control; `--keep 1` keeps B as the newest, by design). Three distinct boot ids prove the two kexecs (the console is informational: a generation boots on the PL011 after the helper rewrites its `console=`); each trial's output is echoed into the run log. Green end to end 2026-09-02, and with the pin steps 2026-09-04. Cannot prove the SoC (kexec on RK3566 with the BSP firmware, the panel and PMIC through a warm restart, brcmfmac over SDIO): those stay UART-attended | `make qemu-update-check ROOTFS=…` |
| `pinenote/tools/update-path` | 1 | The update path's pure decisions (`doc/update-path.md`), run on the packaged `generation_ledger.lua`: Guix's `parameters` sexp read from the REAL v0.2.0-lineage system (kernel, initrd, all ten kernel arguments), `append_for` reproducing the shipped extlinux `APPEND` byte for byte, the generations menu rendered newest-first with `DEFAULT` = the *promoted* generation (never implicitly the newest — the trial-then-promote invariant), refusal of a default outside the ledger, `prune_plan` keeping K newest and always the promoted, booted and pinned ones (a pin inside and outside the window, on the promoted one, all pinned, no set), `pin_path` inside `/boot/gen-N`, the health predicate matrix, and `booted_system` off a command line. Structural pins on the imperative halves: the trial boot's teardown order (reader stopped → radio off → gadget unbound → EBC idle → `kexec -l` → sync → remount ro → `kexec -e`, exactly once), `add` materializing payloads for pre-helper generations, the helper never naming p7/os1/the table, `pin`/`unpin` writing only the marker in the generation's own payload directory and `prune` handing the pinned set to the planner, the rig pinning/pruning/unpinning around the rollback, and the deployer promoting only after a passing health check | `make update-path-check`; the flow itself is `make deploy DEVICE=…` |
...
| `pinenote/scripts/uart` (`test-uboot-pick-slot.sh`) | 1 | The U-Boot slot picker — the deployer's UART watcher and the hand-run recovery — driven against a pseudo-terminal replaying the **real** bytes of the 2026-09-04 cold boot (`doc/artifacts/pinenote-gen16-deploy-20260904/uart-coldboot.log`: SPL, U-Boot's earlier `CTRL+C` autoboot prompt, the ANSI-drawn slot menu with its mid-line drops, extlinux's own generation menu). Pins the port setup before the first read (1500000, raw, no echo, no flow control, no CR/NL translation, `min 1`), the `LOG.watcher` handle (pid = the script, pgid = pid, reader in that group) under both start shapes — as `make` starts it and as a job-control shell does, where `setsid` forks and `$!` is a dead wrapper — that nothing is sent for the CTRL+C prompt or the generation menu (which shares the title and "Press UP/DOWN" line with the slot menu), the exact keystrokes per slot from the menu's ONE draw (U-Boot draws the entries once and then repeats only the countdown line) even with three of its strings clipped by a 25-byte drop, or with all three entry lines clipped and only the countdown line `Hit any key to stop autoboot` intact — while the pre-menu `Hit key to stop autoboot('CTRL+C')` fed alone is not answered — `exit=` in the handle, and `exit=terminated` (never `exit=0`) for a picker reaped before any menu, under `sh` and under dash or bash when the other is on PATH (bash runs an EXIT trap on an untrapped TERM with `$?` of the last command; dash runs none), the reader outliving the pick, the recorded group reaping it, and the no-menu timeout (`WILKBOOK_UBOOT_MENU_TIMEOUT`). python3 + setsid + stty, eight cases (nine with a second shell on PATH), ~40 s | `make uart-pick-check` |
...
| `pinenote/tools/ebc-lab/turn-check.sh` | 6 (device-side) | The injected-page-turn instrument that avoids the file-manager trap (this file's IRQ-units section): opens the last book for the run, starts the optics injector, restarts the reader, confirms the `opening file` line, injects N `KEY 158` turns and logs per turn the EBC IRQ total, `runtime_status`, an md5 of `/dev/fb0` and the driver's belief hash; ~47 IRQs per real turn on the direct driver, 0 means the page did not change (2026-09-04) | no `make` target; copy to `/data/wilkbook/tools` with `optics-inject.lua`, `belief-grab.lua`, `ebclib.lua` |
| `pinenote/tools/platform-controls/wifi-cycle.sh` | 6 (device-side) | The hands-off suspend/resume rig behind the Wi-Fi-after-resume numbers (`doc/status.md` 2026-09-03/04 and 2026-09-04): one injected power tap, then the broker suspends, RTC-wakes after `backstop`, restores Wi-Fi, settles `rtc_settle` and re-suspends; `koreader-idle N` makes KOReader's own timer the trigger (unplug the cable — the timer never fires while the battery reports charging); `report` judges from the broker log and `wifi.log` for the segment after the last start mark | no `make` target; copy to `/data/wilkbook/tools`, drive it over ssh as its header shows |
...
restart, or use `pinenote/tools/ebc-lab/turn-check.sh` (copied to
`/data/wilkbook/tools`), which opens the last book for the run, hashes
the framebuffer and the driver's belief buffer per turn, and restores
the settings after.
