...
| R3 | clock, as shipped (no server): the timesync service is **inert** — `/var/log/pinenote-timesync.log` carries one `no --server configured: the clock stays whatever the RTC holds` line for this boot and nothing after it, `herd status pinenote-timesync` says stopped, and `date -u`, `/sys/class/rtc/rtc0/since_epoch` and the line's own timestamp agree to within a minute; `dmesg` carries `rk808-rtc: setting system clock to …` — the RTC, not the network, set the clock. **Run 2026-09-04 on generation 14: all of it held, the clocks agreed to the second** | daemon log + `date -u` + `since_epoch` + `herd status` | #27's shipped default |
