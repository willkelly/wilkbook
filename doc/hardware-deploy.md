...
last-trial` are the same verbs; `deploy.sh DEVICE --rollback N` is
trial+health+promote of an existing generation. The bail-out lives in
the helper the *target* generation ships (the deployer runs
