...
`promote`, `demote`, `pin`, `unpin`, `prune`, `health`, `last-trial`. The generation ledger is
...
- **Rung 4 — the whole flow in QEMU virt** (`make qemu-update-check ROOTFS=…`): boot generation A, `guix
  ...
  the stale refusal record does not, `promote`, `pin` A and `prune
  --keep 1` (A survives; `unpin`), then roll back to A and `prune --keep
  0` (B goes). The mechanism end-to-end, no glass; green with the pin
  steps 2026-09-04 (three boot ids, gen-1's marker and payload intact
  through the pinned prune, gen-2 gone after the unpinned one).
...
- **Recovery, updated.** A trial that halts now self-recovers by
  ...
  The picker's menu match, its port setup and the
