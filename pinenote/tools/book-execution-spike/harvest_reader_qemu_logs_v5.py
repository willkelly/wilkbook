...
def open_pending_manifest(directory_fd: int) -> int:
    return os.open(
        "HARVEST.txt",
        os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_CLOEXEC,
        0o600,
...
def write_open_manifest(fd: int, content: bytes) -> None:
    os.ftruncate(fd, 0)
    ...
    while offset < len(content):
        written = os.write(fd, content[offset:])
        if written <= 0:
            raise HarvestError("short canonical manifest write")
        offset += written
    os.fsync(fd)


def neutralize_failed_output(stream: BinaryIO) -> None:
    """Drain buffered bytes to /dev/null after a caught output failure."""
...
def publish_output(stream: BinaryIO, content: bytes) -> str | None:
    """Explicitly write and flush one bounded stream without shutdown EPIPE."""
    try:
        stream.flush()
        if content:
            ...
            if written != len(content):
                raise HarvestError(
                    f"short final output write: {written!r} of {len(content)} bytes"
                )
        stream.flush()
        return None
    except (HarvestError, OSError, ValueError) as error:
        neutralize_failed_output(stream)
        return str(error)
...
def run(arguments: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Create the canonical path in a fail-closed pending state, then make
        # both it and its directory read-only before attempting final output.
        # The already-open private FD remains the sole manifest writer.
        manifest_fd = open_pending_manifest(evidence_fd)
        ...
        # Required launcher output is replayed and explicitly flushed before a
        # canonical complete manifest can exist.  Stderr goes first so its
        # failure cannot follow a success-looking stdout line.  A caught EPIPE
        # is redirected and drained to /dev/null to prevent Python's implicit
        # shutdown flush from changing the selected status to 120.
        if ready_to_publish:
            publication_error = publish_output(sys.stderr.buffer, launcher_stderr)
            if publication_error is not None:
                add_error(
                    errors,
                    f"required launcher.stderr publication failed: {publication_error}",
                )
            else:
                publication_error = publish_output(sys.stdout.buffer, launcher_stdout)
                if publication_error is not None:
                    add_error(
                        errors,
                        f"required launcher.stdout publication failed: {publication_error}",
                    )
            record_received_signal()

        complete = ready_to_publish and not errors and received_signal is None
        manifest = [
            "schema=2",
        ...
        write_open_manifest(manifest_fd, ("\n".join(manifest) + "\n").encode())

        if complete:
            return 0
        diagnostics = bytearray(launcher_stderr)
        diagnostics.extend(
            "".join(f"HARVEST: FAIL:{error}\n" for error in errors).encode()
        )
        publish_output(sys.stderr.buffer, bytes(diagnostics))
        return direct_status if direct_status not in (None, 0) else 1
    except (HarvestError, OSError) as error:
        # Once an exclusive evidence directory exists, retain a minimal failed
        # manifest even when an internal host operation aborts the normal path.
        # Preflight refusal before evidence creation intentionally remains
        # artifact-free.  Cleanup status never replaces an observed nonzero
        # direct-launcher status.
        add_error(errors, str(error))
        force_close_pipes()
        if process is not None:
            ...
            except (HarvestError, OSError) as cleanup_error:
                add_error(errors, f"emergency owned-tree cleanup failed: {cleanup_error}")
            polled = process.poll()
            if polled is not None and direct_status is None:
                direct_status = normalize_returncode(polled)
        if evidence_fd is not None:
            fallback = [
                ...
                f"launcher-status={direct_status if direct_status is not None else 'not-started-or-unreaped'}",
                "writer-completion=false",
                "emergency-failure-manifest=true",
                f"adopted-owned-children-observed={len(adopted_seen)}",
                f"adopted-owned-children-unreaped={len(adopted)}",
                *[f"error={sanitized_error(message)}" for message in errors],
            ]
            try:
                fallback_content = ("\n".join(fallback) + "\n").encode()
                if manifest_fd is not None:
                    write_open_manifest(manifest_fd, fallback_content)
                elif safe_stat("HARVEST.txt", evidence_fd) is None:
                    write_evidence(
                        evidence_fd,
                        "HARVEST.txt",
                        fallback_content,
                ...
                os.fchmod(evidence_fd, 0o500)
            except (HarvestError, OSError) as publication_error:
                add_error(
                    errors,
                    f"emergency failure-manifest publication failed: {publication_error}",
                )
        publish_output(
            sys.stderr.buffer,
            "".join(f"HARVEST: FAIL:{message}\n" for message in errors).encode(),
        )
        return direct_status if direct_status not in (None, 0) else 1
    finally:
        selector.close()
        ...
        for log in held.values():
            try:
                ...
                pass
        for fd in (manifest_fd, reader_fd, root_fd, evidence_fd, base_fd):
            if fd is not None:
                try:
                    os.close(fd)
                except OSError:
