...
HARVESTER = HERE / "harvest_reader_qemu_logs_v5.py"
SUCCESS_LOOKING = (
...
if args.scenario in ("direct23", "publish-success"):
    os.write(1, %r)
...
class HarvesterTests(unittest.TestCase):
    ...
    def test_success_unlink_after_all_writers_done(self) -> None:
        result, run_base, _records, evidence, _elapsed = self.invoke("publish-success")
        self.assertEqual(result.returncode, 0, result.stderr.decode(errors="replace"))
        self.assertEqual(result.stdout, SUCCESS_LOOKING)
    ...
    def test_post_evidence_exec_failure_retains_failed_manifest(self) -> None:
        _temporary, run_base, records, _fixture, evidence = self.make_case()
        command = [
            sys.executable,
            str(HARVESTER),
            "--run-base",
            ...
            "3",
            "--",
            "/definitely/not/a/reader-qemu-launcher",
            "--run-base",
            str(run_base),
            "--timeout-seconds",
            "600",
        ...
        manifest = (evidence / "HARVEST.txt").read_text(encoding="utf-8")
        self.assertIn("harvest-status=failed", manifest)
        self.assertIn("launcher-status=not-started-or-unreaped", manifest)
        self.assertIn("emergency-failure-manifest=true", manifest)
        self.assertNotIn("harvest-status=complete", manifest)

    def test_closed_final_stdout_fails_closed_without_exit_120(self) -> None:
        _temporary, run_base, records, fixture, evidence = self.make_case()
        read_fd, write_fd = os.pipe()
        try:
            process = subprocess.Popen(
                self.command("publish-success", run_base, records, fixture, evidence),
                stdin=subprocess.DEVNULL,
                stdout=write_fd,
                stderr=subprocess.PIPE,
                close_fds=True,
        ...
        self.assertNotEqual(process.returncode, 120)
        self.assertEqual(list(run_base.iterdir()), [])
        self.assertEqual((evidence / "launcher.stdout").read_bytes(), SUCCESS_LOOKING)
        for name in (
            "console.log",
        ...
        self.assertIn("harvest-status=failed", manifest)
        self.assertNotIn("harvest-status=complete", manifest)
        self.assertIn("required launcher.stdout publication failed", manifest)
        self.assertEqual((evidence / "HARVEST.txt").stat().st_mode & 0o777, 0o400)
        self.assertEqual(evidence.stat().st_mode & 0o777, 0o500)

    def test_stable_reader_rejects_metadata_change_during_exact_read(self) -> None:
        specification = importlib.util.spec_from_file_location("harvester_v5", HARVESTER)
