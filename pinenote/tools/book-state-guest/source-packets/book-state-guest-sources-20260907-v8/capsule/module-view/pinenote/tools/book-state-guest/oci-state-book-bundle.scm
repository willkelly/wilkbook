;;; Fixed Book Protocol OCI bundles for the persistent-state guest join.
;;;
;;; This file is a narrow source successor to the accepted
;;; book-execution-spike/oci-book-bundle.scm.  It restores the accepted Python
;;; -I -S pair and loads one compile-fixed storage-boundary probe immediately
;;; before each unchanged joined book.  It retains every accepted CONTROL/runsc,
;;; mount, language-closure, and FD-donation decision; in particular it does not
;;; mount the authority's persistent volume or private UI transport.
    ...
    ((guile)
     `(("args" . #("/profile/bin/guile" "--no-auto-compile"
       ...
       ("env" . #("BOOK_SESSION_FD=3"
                   ...
                   "TMPDIR=/scratch"))))
    ((python)
     `(("args" . #("/profile/bin/python3" "-I" "-S" "-B" "-c"
                    ,python-entry-code))
       ("env" . #("BOOK_SESSION_FD=3"
                   "HOME=/scratch"
                   "LANG=C.UTF-8"
...
(define (source-destinations kind sources)
  (case kind
    ((guile)
     (match sources
       ((boundary-probe entry protocol blocking)
        (list (cons boundary-probe "/book/storage-boundary.scm")
              (cons entry "/book/entry.scm")
              (cons protocol "/book/modules/book-protocol.scm")
              (cons blocking "/book/modules/book-protocol/blocking-io.scm")))
       (_ (bundle-error "Guile bundle requires exactly four fixed sources"))))
    ((python)
     (match sources
       ((boundary-probe entry protocol)
        (list (cons boundary-probe "/book/storage_boundary.py")
              (cons entry "/book/entry.py")
              (cons protocol "/book/modules/book_protocol.py")))
       (_ (bundle-error "Python bundle requires exactly three fixed sources"))))
    (else (bundle-error "unknown fixed protocol-book kind"))))
...
(define* (generate-guile-protocol-bundle
           #:key profile-input boundary-probe-input book-entry-input
           protocol-input blocking-input bundle-input container-id
           requisites-runner
  ...
  (generate-fixed-protocol-bundle
   #:kind 'guile
    ...
    #:source-inputs
    `((,boundary-probe-input . "Guile fixed storage-boundary probe")
...
(define* (generate-python-protocol-bundle
           #:key profile-input boundary-probe-input book-entry-input
           protocol-input bundle-input container-id requisites-runner
           (store-root "/gnu/store"))
  (generate-fixed-protocol-bundle
   #:kind 'python
    #:profile-input profile-input
    #:source-inputs
    `((,boundary-probe-input . "Python fixed storage-boundary probe")
