...
Status: **non-shipping v8 source candidate**. Reader-join v2 and the rebased
...
V7 changed the authority and host checks only to validate and publish each
actual finalized record; its exact source received finite independent acceptance
and one source-derived image was built, but no runtime acceptance followed.
V8 preserves those marker bytes and adds only a typed completed-book-result join
to each trusted attribution. It is not yet independently accepted, built, or
booted.
...
- Candidate repository baseline: `399764fa53d4e54bdcfaf84c362d4d1debf35f4c`.
  ...
  source base `549dded816e5f73d2c11557ffcd3130a018b82a8`; v8 does not rewrite it.
...
entire runsc process group, and both bounded pipes have reached EOF, v8 requires
...
```text
BOOK-STATE-GUEST sandbox-boundary-source language=LANG container=CONTAINER source=owned-finalized-runsc.stdout publication=next-line-after-child-drain marker-bytes=N marker-sha256=HEX64 capture-bytes=N capture-sha256=HEX64 operation=OPERATION resulting-state-version=VERSION
```

`LANG` and `CONTAINER` are the compile-fixed matching identity; byte counts and
digests come from that stable capture read. `OPERATION` and `VERSION` are a
private copy of the operation ID and final version returned by the same
`drive-fixed-language!` invocation after
`reader-save-completion->decision` validates its typed commit receipt. The
operation must satisfy the closed 1–128-byte ASCII Book State grammar and the
version must be the actual absent→A version 1 or A→B version 2 transition. A
missing or malformed result, a missing operation in either save phase, or a
wrong transition/version fails before attribution is published. These fields do
not come from database readback, a console parse, or an expected fixture value.

The accepted third, stable-B invocation performs no save. It has the distinct
closed attribution suffix `read-version=2 no-new-commit=true`; it never invents
an operation ID and cannot satisfy the first-two-boot commit grammar. Only after
the source-record write succeeds does
...
accepted owner joins runsc and finalizes its bounded captures. V8 validates and
...
Both save versions for both languages prove that model-console bytes come from
the finalized capture, carry its digest/length, and carry the passed completed
book result's distinct operation/version. The stable-B model proves the separate
read-only grammar without an invented operation. Missing or invalid completed
results, missing save operations, wrong versions, wrong-language/container, duplicate,
