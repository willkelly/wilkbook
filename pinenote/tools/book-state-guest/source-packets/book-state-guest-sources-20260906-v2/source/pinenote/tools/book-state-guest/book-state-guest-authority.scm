;;; Finite trusted guest authority for the two-boot persistent-note proof.
;;;
...
(define-record-type <state-world>
  (make-state-world endpoint child control generation whole-deadline
  ...
  (whole-deadline world-whole-deadline)
...
(define (deadline-after seconds whole-deadline)
  (min whole-deadline (+ (now-seconds) seconds)))
    ...
    (let ((waited
      ...
      (if (not waited)
          ...
          (unless (and (= (car waited) pid)
                       ...
                       (= (status:stop-sig (cdr waited)) SIGSTOP))
            (fail "runsc FD adapter did not stop before runtime exec"))))))
...
(define (pump-world! world)
  (check-deadline! (world-whole-deadline world) "whole guest run")
...
(define (await-operation-completion! world label)
  (await-world!
   ...
   (deadline-after operation-timeout-seconds (world-whole-deadline world))
   label))
...
(define (drive-fixed-language! world language)
  (let ((selected (profile language)))
    ...
    (await-world!
     ...
     (deadline-after ui-phase-timeout-seconds (world-whole-deadline world))
...
(define (run-one-sandbox-book! runtime control language bundle whole-deadline
                               supervisor-guile fd-adapter)
  (let* ((selected (profile language))
...
(define (finish-ui! control generation whole-deadline)
  (let ((ticket (queue-book-ui-command! control 'finish generation "")))
    (let flush ()
      (check-deadline! whole-deadline "UI finish delivery")
      ...
      (unless (book-ui-command-delivered? control ticket)
        ...
        (flush))))
  (wait-for-ui-event! control generation whole-deadline 'done "ok"
                      "UI done acknowledgement")
  (let loop ()
    (check-deadline! whole-deadline "UI EOF")
    (pump-book-ui-output! control)
    (let ((event (pump-book-ui-input! control)))
      (cond
       ((eq? event 'eof) #t)
...
(define (run-guest! config)
  ...
  (define whole-deadline (+ (now-seconds) whole-run-timeout-seconds))
  (define control #f)
  (define runtime #f)
      ...
      (let ((closure (assert-source-provenance! config)))
        ...
        (call-with-values
          ...
          (lambda (guile-bundle python-bundle)
            ...
            (set! control (open-book-ui-control! whole-deadline))
            (wait-for-ui-event!
             control 1 whole-deadline 'channel-ready "" "UI channel readiness")
            (set! runtime (open-reader-state-runtime state-root))
            (let ((results
                   (list
                    (run-one-sandbox-book!
                     runtime control 'guile guile-bundle whole-deadline
                     (field config 'supervisor-guile)
                     (field config 'runsc-fd3-adapter))
                    (run-one-sandbox-book!
                     runtime control 'python python-bundle whole-deadline
                     (field config 'supervisor-guile)
                     (field config 'runsc-fd3-adapter)))))
              ...
              (finish-ui! control 2 whole-deadline)
              (marker "result=pass initial-stage=~a final-versions=~a,~a"
                      (result-value (car results) 'initial)
