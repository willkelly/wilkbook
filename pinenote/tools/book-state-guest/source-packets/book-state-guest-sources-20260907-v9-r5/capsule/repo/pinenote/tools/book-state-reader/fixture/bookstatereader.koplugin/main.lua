...
function Probe:_dismiss_startup_overlays()
    if self.startup_overlays_dismissed then return true end
    local dismissed = 0
    local seen = {}
    local top = UIManager:getTopmostVisibleWidget()
    while top and top ~= self.note_dialog and dismissed < 8 do
    ...
    if not self:_check(top == self.note_dialog,
            "could not make InputDialog topmost")
            or not self:_check(dismissed == 2,
