...
def main() -> None:
    ...
    check(
        ...
        "v5 names the current candidate baseline without rewriting prerequisite history",
    )
    check(
    ...
    authority_without_probe_wiring = authority.replace(
        "     #:boundary-probe-input (field config 'guile-boundary-probe)\n", "", 1
    ).replace(
        "     #:boundary-probe-input (field config 'python-boundary-probe)\n", "", 1
    )
    check(
        hashlib.sha256(authority_without_probe_wiring.encode("utf-8")).hexdigest()
        == "688b2a9dbe5d9ce983cb3e8ca5604a12f494a6a592f5cf0b6cede4fbbeb3be73",
        "authority differs from accepted BSG-1/BSG-2 bytes only by fixed probe wiring",
    )
