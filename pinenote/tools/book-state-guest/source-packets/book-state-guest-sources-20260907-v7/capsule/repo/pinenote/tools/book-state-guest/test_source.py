...
def main() -> None:
    ...
    check(
        ...
        in system
        and "guest-source-successor=v7-finalized-capture-boundary-observer"
        in system
        ...
        "v7 names the current candidate baseline without rewriting prerequisite history",
    )
    check(
    ...
    v6_record_tail = """    (owned-runsc-finalized?
     . ,(@@ (guest-book-protocol) owned-runsc-finalized?))))"""
    v7_record_tail = """    (owned-runsc-finalized?
     . ,(@@ (guest-book-protocol) owned-runsc-finalized?))
    ...
    (protocol-command-result-stdout-overflow?
     . ,(@@ (guest-book-protocol)
            protocol-command-result-stdout-overflow?))))"""
    authority_without_relay = authority.replace(
        "  #:use-module (ice-9 binary-ports)\n", "", 1
    ).replace(v7_record_tail, v6_record_tail, 1)
    authority_without_relay, relay_blocks = re.subn(
        r"\n;;; BEGIN V7 FINISHED-CAPTURE SANDBOX-BOUNDARY RELAY\n.*?"
        r";;; END V7 FINISHED-CAPTURE SANDBOX-BOUNDARY RELAY\n",
        "",
        authority_without_relay,
        count=1,
        flags=re.S,
    )
    authority_without_relay = authority_without_relay.replace(
    ...
    check(
        relay_blocks == 1
        and hashlib.sha256(authority_without_relay.encode("utf-8")).hexdigest()
        == "e1bc1c871b0502d17ccdc7056564989cd455fae943133300df88e60e11dbdf3c",
        "authority differs from accepted v6 only by the finalized-capture relay",
    )
    ...
    check(
        ...
        "real per-language join publishes only after reap/drain/result validation and before cleanup",
    )
    relay = authority[
        authority.index(";;; BEGIN V7 FINISHED-CAPTURE") :
        authority.index(";;; END V7 FINISHED-CAPTURE")
    ]
    check(
        all(
            token in relay
        ...
        "relay reads the actual stable bounded capture and emits only its exact validated record",
    )
    check(
    ...
    check(
        ...
        "authority and system share the exact persistent-volume and sentinel contract",
    )
    check(
    ...
    check(
        ...
        "host relay matrix drives the actual owned capture and authority helper",
    )
    check(
