...
- `module-view`: only the transitive `(pinenote ...)` modules and explicit
  relative `local-file` assets needed while those modules evaluate, including
  both fixed in-sandbox storage-boundary probes;
...
QEMU, runsc, ARM code, networking, mounts, devices, SSH, or hardware. In
particular, parsing/compiling the fixed boundary probes on the host is not proof
that their storage and UI-path opens are denied inside the future ARM sandbox.
