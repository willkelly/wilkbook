...
(define (model-commit-result language operation version)
  `((language . ,language)
    (initial . ,(if (= version 1) 'absent 'a))
    (initial-version . ,(- version 1))
    (initial-text . "model-initial")
    (final-version . ,version)
    (final-text . "model-final")
    (operation-id . ,(string-copy operation))))

(define (model-read-result language)
  `((language . ,language)
    (initial . b)
    (initial-version . 2)
    (initial-text . "model-B")
    (final-version . 2)
    (final-text . "model-B")
    (operation-id . #f)))
...
(define (run-finalizer-model name expression)
  (let* ((root (model-case-root name))
         ...
         (child
          ((base-private 'spawn-owned-runsc)
           ...
           (list host-guile "--no-auto-compile" "-c" expression)
    ...
    ((base-private 'finalize-owned-runsc!) child)
    `((status . ,((@@ (guest-book-protocol) owned-runsc-status) child))
      (group-live? . ,((base-private 'process-group-exists?)
                       ((@@ (guest-book-protocol)
                            owned-runsc-process-group) child)))
      (elapsed . ,(/ (- (get-internal-real-time) started)
                     internal-time-units-per-second 1.0))
      (stdout . ,(call-with-input-file stdout get-string-all))
      (stderr . ,(call-with-input-file stderr get-string-all)))))

(define* (run-relay-model case-name scenario language fixture-language mutation
                          #:optional
                          (completed-book-result
                           (model-commit-result language
                                                (format #f "model_~a_v1"
                                                        language)
                                                1)))
              ...
              (lambda ()
                ((@@ (book-state-guest-authority)
                 ...
                 (if (eq? mutation 'wrong-container)
                  ...
                  root child result completed-book-result
                  model-console-emit)))))
        `((failed? . ,failed?)
          (emitted . ,emitted)
          (console . ,console)
...
(let ((result
       (run-finalizer-model
        "natural-finalizer"
        "(begin (display \"natural-out\") (force-output) (usleep 150000) (exit 0))")))
  (test-equal "finalizer permits delayed natural runsc success"
    '(exit . 0) (model-value result 'status))
  (test-assert "natural finalizer reaps the exact process group"
    (not (model-value result 'group-live?)))
  (test-equal "natural finalizer drains stdout before returning"
    "natural-out" (model-value result 'stdout))
  (test-equal "natural finalizer retains empty stderr"
    "" (model-value result 'stderr)))

(let ((result
       (run-finalizer-model
        "stuck-finalizer"
        "(let loop () (sleep 60) (loop))")))
  (test-equal "stuck finalizer still escalates to SIGTERM"
    `(signal . ,SIGTERM) (model-value result 'status))
  (test-assert "stuck finalizer reaps the exact process group"
    (not (model-value result 'group-live?)))
  (test-assert "stuck finalizer remains bounded after natural grace"
    (let ((elapsed (model-value result 'elapsed)))
      (and (>= elapsed 2.5) (< elapsed 6.0)))))
...
(define (assert-valid-commit-relay-model language operation version)
  (let* ((case-name
          (format #f "valid-~a-v~a" language version))
         (completed (model-commit-result language operation version))
         (result
          (run-relay-model case-name "valid" language language 'none completed))
         ...
         (expected-source
          (format #f
                   "BOOK-STATE-GUEST sandbox-boundary-source language=~a container=~a source=owned-finalized-runsc.stdout publication=next-line-after-child-drain marker-bytes=~a marker-sha256=~a capture-bytes=~a capture-sha256=~a operation=~a resulting-state-version=~a"
...
(assert-valid-commit-relay-model 'guile "model_guile_a" 1)
(assert-valid-commit-relay-model 'python "model_python_a" 1)
(assert-valid-commit-relay-model 'guile "model_guile_b" 2)
(assert-valid-commit-relay-model 'python "model_python_b" 2)

(let* ((result
        (run-relay-model "valid-guile-read-only-B" "valid" 'guile 'guile
                         'none (model-read-result 'guile)))
       (emitted (model-value result 'emitted)))
  (test-assert "stable-B read-only relay accepts actual read version without inventing an operation"
    (and (not (model-value result 'failed?))
         (= (length emitted) 2)
         (string-suffix? "read-version=2 no-new-commit=true" (car emitted))
         (not (string-contains (car emitted) "operation=")))))
...
(for-each
 (lambda (case)
   (let ((result
          (run-relay-model (car case) "valid" 'guile 'guile 'none
                           (cadr case))))
     (test-assert (string-append (car case) " fails before trusted attribution")
       (model-value result 'failed?))
     (test-equal (string-append (car case) " emits no trusted line")
       '() (model-value result 'emitted))))
 `(("missing-completed-book-result" ,#f)
   ("invalid-result-operation"
    ,(model-commit-result 'guile "not.valid" 1))
   ("wrong-result-version"
    ,`((language . guile)
       ...
       (final-version . 2)
       (final-text . "model-final")
       (operation-id . "model_guile_wrong_version")))
   ("missing-save-operation-in-save-phase"
    ,`((language . guile)
       (initial . absent)
       (initial-version . 0)
       (initial-text . "model-initial")
