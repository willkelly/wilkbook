...
(define %base-record-procedures
  `((make-protocol-peer
    ...
    (protocol-command-result-status
     . ,(@@ (guest-book-protocol) protocol-command-result-status))
    (protocol-command-result-stdout-observed-bytes
     . ,(@@ (guest-book-protocol)
            protocol-command-result-stdout-observed-bytes))
    (protocol-command-result-stdout-overflow?
     . ,(@@ (guest-book-protocol)
            protocol-command-result-stdout-overflow?))))
...
;;; BEGIN V8 BOOK-RESULT-BOUND FINISHED-CAPTURE SANDBOX-BOUNDARY RELAY
;; The authenticated probes write their records before the fixed books start,
;; but stdout is an owned diagnostic capture rather than a serial channel.  A
;; successful book lifetime must therefore finish and drain before this trusted
;; authority can publish the actual captured record to the outer observer.
(define max-owned-capture-bytes (base-private 'max-capture-bytes))
(define boundary-marker-stem "BOOK_STATE_SANDBOX_BOUNDARY")

(define (same-capture-identity? left right)
...
(define (read-finalized-stdout-capture! path expected-bytes)
  (unless (and (integer? expected-bytes) (exact? expected-bytes)
               (<= 0 expected-bytes max-owned-capture-bytes))
    (fail "finalized stdout capture has an impossible observed-byte count"))
  (let ((path-before
         (catch 'system-error
           (lambda () (lstat path))
           (lambda arguments
             (fail "finalized stdout capture is missing: ~a" path)))))
    (unless (and (eq? (stat:type path-before) 'regular)
                 (= (stat:nlink path-before) 1)
                 (= (stat:uid path-before) (getuid))
                 (= (stat:gid path-before) (getgid))
                 (= (logand (stat:mode path-before) #o7777) #o600)
                 (= (stat:size path-before) expected-bytes))
      (fail "finalized stdout capture has the wrong ownership/type/mode/size"))
    (let* ((descriptor
            (catch 'system-error
              (lambda ()
                (open-fdes path (logior O_RDONLY O_CLOEXEC O_NOFOLLOW)))
              (lambda arguments
                (fail "could not open finalized stdout capture by stable FD"))))
           (port (fdopen descriptor "rb")))
      (dynamic-wind
        (lambda () #t)
        (lambda ()
          (let ((fd-before (stat descriptor)))
            (unless (same-capture-identity? path-before fd-before)
              (fail "stdout capture identity changed while opening it"))
            (let* ((bytes (get-bytevector-all port))
                   (fd-after (stat descriptor))
                   (path-after
                    (catch 'system-error
                      (lambda () (lstat path))
                      (lambda arguments
                        (fail "stdout capture path disappeared while reading")))))
              (unless (and (= (bytevector-length bytes) expected-bytes)
                           (same-capture-identity? fd-before fd-after)
                           (same-capture-identity? fd-after path-after))
                (fail "stdout capture changed during its bounded stable-FD read"))
              bytes)))
        (lambda () (close-port-quietly! port))))))

(define (substring-count text needle)
...
(define (expected-boundary-marker language)
  (format #f
          "BOOK_STATE_SANDBOX_BOUNDARY: language=~a result=pass storage-mount=absent storage-fd=absent ui-transport=absent book-session-fd=3"
           (profile-value (profile language) 'label)))

(define completed-book-result-fields
  '(language initial initial-version initial-text final-version final-text
             operation-id))

(define (completed-book-result-boundary-suffix! language result)
  ;; RESULT is the private value returned by this same language lifetime's
  ;; drive-fixed-language! call.  In save phases its operation/version came
  ;; directly from reader-save-completion->decision after the typed receipt was
  ;; checked; no database readback, console parse, or fixture constant enters
  ;; this attribution.  The stable-B read-only phase has a separate grammar and
  ;; cannot be mistaken for either required campaign save.
  (unless (and (list? result)
               (every pair? result)
               (equal? (map car result) completed-book-result-fields)
               (eq? (field result 'language) language))
    (fail "~a boundary evidence lacks its exact completed book result"
          (profile-value (profile language) 'label)))
  (let ((initial (field result 'initial))
        (initial-version (field result 'initial-version))
        (final-version (field result 'final-version))
        (operation-id (field result 'operation-id)))
    (cond
     ((or (eq? initial 'absent) (eq? initial 'a))
      (let ((expected-initial-version (if (eq? initial 'absent) 0 1))
            (expected-final-version (if (eq? initial 'absent) 1 2)))
        (unless (and (integer? initial-version) (exact? initial-version)
                     (= initial-version expected-initial-version)
                     (integer? final-version) (exact? final-version)
                     (= final-version expected-final-version)
                     (book-state-wire-operation-id? operation-id))
          (fail "~a completed save result has invalid operation/version attribution"
                (profile-value (profile language) 'label)))
        (format #f "operation=~a resulting-state-version=~a"
                operation-id final-version)))
     ((eq? initial 'b)
      (unless (and (integer? initial-version) (exact? initial-version)
                   (= initial-version 2)
                   (integer? final-version) (exact? final-version)
                   (= final-version 2)
                   (not operation-id))
        (fail "~a completed read-only result has invalid version attribution"
              (profile-value (profile language) 'label)))
      "read-version=2 no-new-commit=true")
     (else
      (fail "~a completed book result has an unknown state transition"
            (profile-value (profile language) 'label))))))

(define (extract-exact-boundary-marker! bytes language)
  (let* ((text
          (catch #t
            (lambda () (utf8->string bytes))
            (lambda arguments
              (fail "finalized stdout capture is not valid UTF-8"))))
         (expected (expected-boundary-marker language))
         (start (string-contains text boundary-marker-stem)))
    ;; Search the whole bounded capture, not an arbitrary matching substring.
    ;; Any quote, duplicate, wrong-language, failure, malformed, or truncated
    ;; appearance of the reserved stem makes the fixed-fixture result unusable.
    (unless (and start
                 (= (substring-count text boundary-marker-stem) 1)
                 (or (zero? start)
                     (char=? (string-ref text (- start 1)) #\newline))
                 (<= (+ start (string-length expected))
                     (string-length text))
                 (string=?
                  (substring text start (+ start (string-length expected)))
                  expected)
                 (< (+ start (string-length expected)) (string-length text))
                 (char=? (string-ref text (+ start (string-length expected)))
                         #\newline))
      (fail "stdout lacks exactly one complete exact ~a boundary record"
            (profile-value (profile language) 'label)))
    ;; Return the bytes extracted from the capture.  The publisher must never
    ;; substitute EXPECTED, even though equality above closed the grammar.
    (substring text start (+ start (string-length expected)))))

(define* (validate-and-publish-sandbox-boundary!
          language container-id bundle child result completed-book-result
          #:optional (emit-line (lambda (line) ((smoke-private 'emit) line))))
  (let* ((selected (profile language))
         (label (profile-value selected 'label))
         (expected-container (profile-value selected 'container-id)))
    (unless (and (procedure? emit-line)
                 (string=? container-id expected-container)
                 child
                 (child-value child 'owned-runsc-finalized?)
                 (equal? (child-value child 'owned-runsc-status) '(exit . 0))
                 (= ((base-record-procedure 'protocol-command-result-status)
                     result)
                    0)
                 (not ((base-private 'capture-overflow?) result))
                 (not ((base-record-procedure
                        'protocol-command-result-stdout-overflow?)
                       result)))
      (fail "~a boundary evidence was requested before bounded runsc success"
            label))
    (let* ((observed
            ((base-record-procedure
              'protocol-command-result-stdout-observed-bytes)
             result))
           (capture-path (string-append bundle "/runsc.stdout"))
           (bytes (read-finalized-stdout-capture! capture-path observed))
           (captured-marker (extract-exact-boundary-marker! bytes language))
           (marker-sha256
            (bytevector->base16-string
             (sha256 (string->utf8 captured-marker))))
            (capture-sha256
             (bytevector->base16-string (sha256 bytes)))
            (book-result-suffix
             (completed-book-result-boundary-suffix!
              language completed-book-result))
            (source-record
             (format #f
                     "BOOK-STATE-GUEST sandbox-boundary-source language=~a container=~a source=owned-finalized-runsc.stdout publication=next-line-after-child-drain marker-bytes=~a marker-sha256=~a capture-bytes=~a capture-sha256=~a ~a"
                     label container-id
                     (bytevector-length (string->utf8 captured-marker))
                     marker-sha256 (bytevector-length bytes) capture-sha256
                     book-result-suffix)))
      ;; Both lines are computed only after all validation.  Attribution has no
      ;; pass token and is published first.  Only a successful second write can
      ;; publish the complete newline-terminated pass marker, after which this
      ;; helper returns and allows later cleanup/final guest success.  The marker
      ;; argument is the actual stable-FD capture substring, never EXPECTED.
      (emit-line source-record)
      (emit-line captured-marker)
      `((marker . ,captured-marker)
        (source . ,source-record)
        (marker-sha256 . ,marker-sha256)
        (capture-bytes . ,(bytevector-length bytes))
        (capture-sha256 . ,capture-sha256)))))
;;; END V8 BOOK-RESULT-BOUND FINISHED-CAPTURE SANDBOX-BOUNDARY RELAY
    ...
    (let* ((endpoint (world-endpoint world))
      ...
      (expect-ui! world 'closed "")
       (let ((final-version (if decision (list-ref decision 2) version))
             ...
             ;; Retain a private copy of the already validated typed-receipt ID.
             ;; No later UI, child, cleanup, or caller object can replace the
             ;; exact text passed to the boundary publisher.
             (operation-id
              (and decision (string-copy (list-ref decision 1)))))
        (when decision
         ...
         (lambda ()
           ...
           (unless (and (equal?
            ...
            (validate-and-publish-sandbox-boundary!
             language container-id bundle
             (peer-value peer 'protocol-peer-child) result scenario)
