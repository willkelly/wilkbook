...
(define (dispatch-decoded! endpoint peer-message)
  (let ((message (peer-message-value peer-message)))
    (if (string=? (field message "type") "hello")
        ...
        (accept-present! endpoint message))))
...
(define (dispatch-if-current! endpoint identity peer-message)
  (with-endpoint-owner
   ...
   (lambda ()
     (and (endpoint-current? endpoint identity)
          (cons 'committed (dispatch-decoded! endpoint peer-message))))))
    ...
    (with-endpoint-owner
     ...
     (lambda ()
       (let* ((identity
              (endpoint-binding-identity
                (session-endpoint-binding endpoint)))
              (queue (endpoint-output-queue endpoint))
         ...
         (when (or (>= (length queue) max-outbound-frames)
         ...
         (set-endpoint-output-queue!
          endpoint
          (append queue (list (%make-outbound-frame frame 0))))
...
(define (close-session! endpoint)
  (let ((close-transport?
         (with-endpoint-owner
          endpoint
          (lambda () (close-one-session-transition! endpoint)))))
    (when close-transport?
      (shutdown-and-close-transport! endpoint))))
