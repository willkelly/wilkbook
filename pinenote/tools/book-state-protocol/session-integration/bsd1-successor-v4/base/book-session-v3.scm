             ...
             (lambda ()
               (cond
                ...
                ((not (state-output-reservation-available? endpoint))
                 (close-book-state-session-delegate-local!
                  delegate 'state-output-backpressure)
                 (set! close? #t)
                 'closing)
                (else
                 (let ((response
                   ...
                   (if (book-state-session-delegate-closing? delegate)
                       (begin (set! close? #t) 'closing)
                       (begin
                         (queue-state-response-under-owner!
                          endpoint delegate response)
                         'queued))))))))
          (lambda arguments
            ;; Protocol V2 has already invalidated its binding for an
            ;; impossible backend result.  Pair that with outer close after
            ;; leaving the endpoint authority mutex.
            (set! close? #t)
            'closing)))
    (when close?
      (close-session! endpoint))
    (if close? 'closed 'complete)))
