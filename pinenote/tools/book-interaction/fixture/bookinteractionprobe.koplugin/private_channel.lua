-- Bounded nonblocking channel used only between the trusted fixture plugin and
-- its Guile host. This is deliberately not Book Protocol and is never donated
-- to the fixture book process.
...
local C = ffi.C
assert(ffi.os == "Linux", "private fixture channel requires Linux fcntl flags")
local F_GETFL = 3
local F_SETFL = 4
...
local function hex_decode(value)
    ...
    if #result > MAX_VALUE_BYTES then
        return nil, "private control value exceeds its byte limit"
    end
    if not valid_utf8(result) then
        return nil, "private control value is not well-formed UTF-8"
    end
    return result
end
...
local function encode_line(kind, generation, value)
    ...
    if type(value) ~= "string" or #value > MAX_VALUE_BYTES
            or not valid_utf8(value) then
        return nil, "private control event value is invalid"
    end
...
PrivateChannel.__index = PrivateChannel

function PrivateChannel:new(options)
...
function PrivateChannel:_report_error(message)
    if self.closed then return end
    if self.on_error then self.on_error(message) end
end

function PrivateChannel:_pump_output()
    local spent = 0
    local completed = 0
...
function PrivateChannel:send(kind, generation, value)
...
function PrivateChannel:waitEvent()
