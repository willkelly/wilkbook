...
MUTATIONS = [
    ...
    # --- gate B: the three shipped EBC parameter copies ----------------
    ("one build-time EBC parameter copy is changed alone",
     "pinenote/services/ebc.scm",
     "refresh_waveform=6 defio_delay_ms=250",
     "refresh_waveform=4 defio_delay_ms=250",
     "UNKNOWN DIVERGENCE ebc-params:refresh_waveform"),
    ("the set_parameter script disagrees with the modprobe options",
     "pinenote/packages/firmware.scm",
     '(display "set_parameter defio_delay_ms 250\\n" port)',
     '(display "set_parameter defio_delay_ms 50\\n" port)',
     "UNKNOWN DIVERGENCE ebc-params:defio_delay_ms"),
    ("a parameter is dropped from one copy",
     "pinenote/services/ebc.scm",
     " dclk_select=0 refresh_waveform=6",
     " refresh_waveform=6",
     "UNKNOWN DIVERGENCE ebc-params:keyset"),
    ...
    ("the qemu boot-milestone grep expects a waveform nothing ships",
     "pinenote/scripts/qemu/run-virt-assertions.sh",
     "grep -aq 'VIRTCHK-WF-6'",
     "grep -aq 'VIRTCHK-WF-4'",
     "UNKNOWN DIVERGENCE waveform-literal:run-virt-assertions.sh boot "
     "assertion 1"),
    ("the qemu named requirement expects a waveform nothing ships",
     "pinenote/scripts/qemu/run-virt-assertions.sh",
     "require 'live refresh_waveform is GL16' 'VIRTCHK-WF-6'",
     "require 'live refresh_waveform is GL16' 'VIRTCHK-WF-4'",
     "UNKNOWN DIVERGENCE waveform-literal:run-virt-assertions.sh boot "
     "assertion 2"),
    ...
    ("a moved Lua opt table is reported, not skipped",
     ...
     "local options = {",
     "site not found"),
    ("a moved shipped-parameter site is reported, not skipped",
     "pinenote/services/ebc.scm",
     "options rockchip_ebc direct_mode=0",
     "options rockchip-ebc direct_mode=0",
     "site not found"),
