...
# rockchip_ebc requires (doc/direct-mode-adoption.md D1/P1).
#
...
# at boot.
#
#   ebc-clut-install.sh COMPILER SOURCE DESTINATION [STAMP]
#
...
usage='usage: ebc-clut-install.sh COMPILER SOURCE DESTINATION [STAMP]'
compiler=${1:?$usage}
...
# the guarantee that a FAILED compile never replaces a good custom_wf.bin has
# to be this script's, not a property of a program it happens to call.
...
# Bookkeeping only.  A stamp we cannot write means the next boot recompiles,
# which is correct but slower -- not a reason to leave the device without a
# display, so this warns and exits 0 with the CLUT in place.
